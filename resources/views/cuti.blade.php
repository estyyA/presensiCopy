...
@section('content')
    ...
    <div class="card shadow-lg border-0 rounded-lg">
        <div class="card-header text-white d-flex justify-content-between align-items-center"
            ...
            <button class="btn btn-light btn-sm rounded-pill px-3 shadow-sm" data-toggle="modal"
                data-target="#createCutiModal">
                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        ...
                                        $badge = 'badge-info';
                                        if (str_contains($ket, 'tahunan')) {
                                            $badge = 'badge-success';
                                        } elseif (str_contains($ket, 'sakit')) {
                                            $badge = 'badge-danger';
                                        } elseif (str_contains($ket, 'pribadi')) {
                                            $badge = 'badge-warning';
                                        }
                                    @endphp
    ...
    {{-- ✅ Modal Tambah Cuti --}}
    <div class="modal fade" id="createCutiModal" tabindex="-1" role="dialog" aria-labelledby="createCutiModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content shadow-lg border-0 rounded-lg">
                <form action="{{ route('cuti.store') }}" method="POST">
                    @csrf
                    <div class="modal-header text-white" style="background: linear-gradient(90deg, #6f42c1, #d63384);">
                        ...
                        <div class="form-group">
                            ...
                            <select name="nik" id="nik" class="form-control" required>
                                ...
                                @foreach ($karyawan as $k)
                                    <option value="{{ $k->NIK }}">{{ $k->NIK }} - {{ $k->nama_lengkap }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
...
@push('styles')
    <style>
        .bg-purple {
            background-color: #3f71dc !important;
        }
