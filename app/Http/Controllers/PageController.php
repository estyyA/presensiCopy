...
=======
// ====== PRESENSI ADMIN (SATU FORM) ======
public function showPresensiAdmin()
{
    return view('admin.presensi');
}

public function submitPresensiAdmin(Request $request)
{
    $admin = session('karyawan');
    if (!$admin || $admin->role !== 'admin') {
        return redirect()->route('login.form')->withErrors(['login' => 'Silakan login sebagai admin']);
    }
    ...
    if ($tipe === 'masuk') {
        ...
        if ($sudahAbsen) {
            return redirect()->route('admin.presensi.form')->with('warning', 'Anda sudah absen masuk hari ini!');
        }

        DB::table('presensi')->insert([
            'NIK'           => $admin->NIK,
            'nama_karyawan' => $admin->nama_lengkap,
            'divisi'        => $admin->nama_divisi ?? '-',
            'tgl_presen'    => $tanggal,
    ...
    return redirect()->route('admin.presensi.form')->with('warning', 'Tipe presensi tidak valid.');
}
// Halaman form absen masuk
public function masuk()
{
    return view('admin.masuk'); // blade yang tadi kamu buat masuk.blade.php
}

// Proses simpan absen masuk
public function storeMasuk(Request $request)
{
    // logika simpan absen masuk di DB
    DB::table('presensi')->insert([
        'NIK'        => auth()->user()->NIK ?? '1234',
        'tgl_presen' => now()->toDateString(),
        'jam_masuk'  => now()->toTimeString(),
        'status'     => 'hadir',
        'created_at' => now(),
        'updated_at' => now(),
